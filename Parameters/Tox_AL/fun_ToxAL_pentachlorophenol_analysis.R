...
penta_data_summary <- penta_data_analysis %>%
  ...
  mutate(critical_excursions = excursions_tox(num_samples),
         Category = case_when(percent_3d == 100 ~ "Cat 3D",
                              num_samples == 1 & num_Violations == 0 ~"Cat3",
                              num_samples == 1 & num_Violations == 1 ~"Cat3B",
                              num_Violations >= critical_excursions ~ "Cat 5", 
                              num_Violations < critical_excursions ~ "Cat 2",
                              TRUE ~ "ERROR"))
