...
IR_Res_qry <- "Select  OrganizationID, [MLocID], Activity_Type, [SampleStartDate],[SampleStartTime],[Char_Name], [Statistical_Base],  Result_UID, [IRResultNWQSunit], AU_ID, act_depth_height
  ...
  WHERE Char_Name in ('Chlorophyll a')"


IR_res <- DBI::dbGetQuery(IR.sql, glue_sql(IR_Res_qry, .con = IR.sql))

# Pull selected data from ResultsRawWater2018. This is used to get act_id
...
View_results <- "SELECT [Result_UID], Char_Name FROM [IntegratedReport].[dbo].[VW_Chl]"
...
grouped_no_res_no_time <- IR_res %>%
  ...
  summarise(num = n(),
            ...
            num_distinct_times = n_distinct(SampleStartTime)) %>%
  filter(num > 1) %>%
  ungroup() %>%
...
save(aggregate_data, file = "Other tools/chl_aggregate_data.Rdata")


# Writedata to IR database for exclusion ----------------------------------

#write csv file of data to be excluded
write.csv(add_to_UnusedData_2018, "Other tools/chl_excluded_datav2.csv")
